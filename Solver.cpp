...
void Solver::work(const String &url, ArrayList<String> &result, const WordDivider &divider, int urlID) {
    {
    ...
    if (result.size() == 9) {
        titleWords = divider.divideWords(result[4]);
        contentWords = divider.divideWords(result[5]);
        String res;
    ...
    */
    ///<@note 为了多线程，一定要先开好
    {
        ...
        for (int row = 1; row < csv.size(); row++) {
            ...
            pool.addTask([this, url, &backPtr, &wordDivider, row]() {
...
ArrayList<DocNode> Solver::searchQuery(const std::string &inputLine) {
    String queryLine = StringConvert::fromStdString(inputLine);
...

    {
        ...
        int m = static_cast<int>(std::unique(querys.begin(), querys.end()) - querys.begin());///<@note 去重
        while (querys.size() > m) {
            querys.pop_back();
        }
    }
    ...
    struct sort_node {
        ...
        sort_node(int urlID, int occurSum, int occurTypes) :
                urlID(urlID), occurSum(occurSum), occurTypes(occurTypes) {}
...
void Solver::solveConsole() {
    ...
    while (std::getline(fin, line)) {
        if (!line.empty()) {
            const auto &res = searchQuery(line);
